...
class TopkBMMCuda(CudaCallable): 
  def __init__(
    ...
    with open(get_absolute_path("bmm_helpers.cu"),'r') as f: ###
      helpers = f.read()
    
    with open(get_absolute_path("topkbmm.cu"),'r') as f: ###
  ...
  def __call__(self, A, B, k=128, dim=1):
    ...
    print(values[0, :100, 0], indices[0, :100, 0])
    indices = indices[:, :, :k]
    values = values[:, :, :k]
