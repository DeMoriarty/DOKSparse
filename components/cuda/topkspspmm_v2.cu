...
__global__ void topkspspmm(
  ...
  SmemTensor2D<float, ARowsPerBlock, MaxNNZPerRow> smemAVals(smemAColInds.endPtr);  //[ARowsPerBlock, maxnnzpr]
  
  SmemTensor1D<float, TPB> smemValueExchange(smemAVals.endPtr);  //[TPB]
  SmemTensor1D<int64_t, TPB> smemIndexExchange(smemValueExchange.endPtr);  //[TPB]
  SmemTensor1D<int, ARowsPerBlock> smemSortTrigger(smemIndexExchange.endPtr); //[ARowsPerBlock]
  SmemTensor1D<float, ARowsPerBlock> smemMinValueExchange(smemSortTrigger.endPtr); //[ARowsPerBlock]
