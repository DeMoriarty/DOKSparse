...
__global__ void hash_map_set_p1(
  ...
  // __shared__ KeyType newKeySmem[keySize];
  extern __shared__ KeyType newKeySmem[];

  // Load newKey
  int nLoadIters = div_ru(keySize, ThreadsPerBlock);
  #pragma unroll
  for (int i=0; i<nLoadIters; i++){
    int offset = i * nLoadIters + tid;
    if (offset < keySize){
      newKeySmem[offset] = pNewKey[keySize * bid + offset];
      // newKeySmem[offset] = pNewKey[0];
      // pLastVisitedNodeStatus[bid] = pNewKey[0];
    }
  }
  __syncthreads();
