...
class TopkSPSPMMCuda(CudaCallable):
  def __init__(self, tpb=256, maxnnzpr=8, a_rows_per_block=1, stack_cap=2):
    super().__init__()
    assert 32 <= tpb <= 1024
    self.tpb = tpb
    ...
    self.smem_size = self.a_rows_per_block * self.maxnnzpr * 12 + self.tpb * 12 + self.a_rows_per_block * 8
