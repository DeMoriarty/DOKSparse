...
class TopkSPSPMMCuda(CudaCallable):
  def __init__(self, tpb=256, maxnnzpr=8, minnnzpr=0, tile_m=1, stack_cap=2, bin_search_ver=3, threads_per_group=32):
    ...
    device_max_smem_per_block = cp.cuda.runtime.getDeviceProperties(self.device.index)["sharedMemPerBlock"]
    for i in range(tile_m, 0, -1):
      smem_size = self.tpb * 12 + i * 8 + i * self.maxnnzpr * 12
      if smem_size <= device_max_smem_per_block:
        self.tile_m = i
      ...
      if i > 1:
        ...
        raise MemoryError(f"requried smem exceeds device limit of {device_max_smem_per_block}")
...
class TopkSPSPMM:
  def __init__(self):
    ...
    self.tile_ms =      [   7, 7,  7,  7,   7,   4,    2]
    self.tpbs = [128, 256, 512, 1024]
    self.timer = ProfilingTimer(False, name="TopkSPSPMM")
    for i in range(len(self.nnzpr_ranges) - 1):
      ...
      for tpb in self.tpbs:
        fn = TopkSPSPMMCuda(tpb=tpb, minnnzpr=minnnzpr, maxnnzpr=maxnnzpr, tile_m=tile_m, threads_per_group=4)
        setattr(self, f"fn_{tpb}_{minnnzpr}_{maxnnzpr}", fn)

  def __call__(self, a : Tensor, b : Tensor, k=128, n_candidates=None) -> torch.namedtuple_values_indices:
    assert isinstance(a, Tensor)
    ...
    topk_inds = sorted_indices[topk_inds]
    stop("final map")
    return torch.namedtuple_values_indices(values=topk_vals, indices=topk_inds)
